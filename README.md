...
| 📖 | **从摘要到全文的渐进式阅读** | 先读摘要判断相关性，满足条件的论文走下载 → 用 Nougat 把每一页写成 Markdown → 插图仍按页面截图并配上图注 → 按块装箱 → 逐块精读并汇总成报告。需要再精读一遍时，只清掉旧报告、复用已经切好的正文片段重新阅读，不必从工作区删文再检索。全文下载或转换失败时自动降级为「基于摘要的精读」，并把失败原因告知用户 |
...
全文阅读还要一个单独的 Nougat 环境（不要装进上面这个主环境）。双击 `start.bat` 时会自动装上依赖，并下载大约 1GB 的模型权重。已经有的会跳过。
想手动准备的话，在项目根目录执行：
...
> 此时只有 `/docs` 和 `/api/*` 可用。发给同事的压缩包里只有这一次的构建结果，没有前端源码，同事看到的界面就是打包当时的页面。要更新界面，在有源码的机器上重新执行打包。
...
`config/system.yaml` 存放系统级默认值和阅读参数：`defaults.llm` 是所有 Agent 档位未声明字段时的兜底生成参数，`paper_retrieval` 是检索数据源密钥，`read` 是阅读与下载参数（缓存目录、连接/下载超时、最大文件大小、PDF 解析器）。
...
PDF 正文由本机的 Nougat 按页写成 Markdown。它读的是整页，公式会写成 LaTeX。写进 `paper.md` 之前，把 Nougat 的 `\[...\]` 换成 `$$`、`\(...\)` 换成 `$...$`，这样后面的分块仍能把公式认成一整块。

`paper.md` 和 `chunk.json` 仍从同一份文字来。精读、追问、综述写作读到的仍是每段里的文字，不另存一份块文件。

一块仍是这几种：标题、段落、独立公式、夹在句子里的公式、表格、图。公式和表格以 Nougat 写出来的样子为准，不再截图交给精读模型重排。

插图不交给 Nougat。仍用原来的办法在页面上找图、配图注、截到 `assets/`，再把 `![图注](assets/...)` 接到对应页的末尾。精读时的插图解读也不变。

Nougat 装在 `tools/nougat_trial` 这个单独环境里。`start.bat` 启动时如果还没有这份环境，会从阿里云装上 PyTorch，并把大约 1GB 的模型权重下到 `tools/nougat_trial/weights`（只留在本机，不提交）。主程序用子进程调用。环境或权重不在时，这次转换直接失败，不再退回按字形抽正文。
...
转换器和分片各自有版本号（现在是 9 和 10）。旧的 `paper.md` / `chunk.json` 对不上就重做。没有现成的块时，分片按空行从 Markdown 认块。
