...
| 📖 | **从摘要到全文的渐进式阅读** | 先读摘要判断相关性，满足条件的论文走下载 → 把一页分成标题、段落、公式、表格、图 → 公式和表格截图转写、插图连着图注阅读 → 按这些块装箱 → 逐块精读并汇总成报告。需要再精读一遍时，只清掉旧报告、复用已经切好的正文片段重新阅读，不必从工作区删文再检索。全文下载或转换失败时自动降级为「基于摘要的精读」，并把失败原因告知用户 |
...
| 🔬 | **分层研究分析** | 综述先把工作区全部论文的结构化摘要做一次整体分析，再做一次全局综合，形成研究现状、共识、争议、空白、时间演化与展望等结构化内容 |
| ✍️ | **证据约束下的综述写作** | 先生成大纲与证据映射，再逐节写作；证据不足时检索补充，写完后审查并限次修改 |
...
| 全文处理 | `PyMuPDF` 先把一页分成标题、段落、公式、表格、图（装不上时退回 `pypdf`，那条路没有块）；独立公式和表格按页面上的矩形截图转写；分片按这些块装箱，不从一块中间切开 |
...
PDF 里的公式不是一段现成的文字，而是一堆带坐标的字形。如果先把一页扫成一篇长文，再用规则去切，圈不干净的字形会变成正文里的 `$...$`，切的时候只顾着保护 `$$` 和表格，半截公式就会被切开。

所以现在倒过来做：一页先分成块，`paper.md` 和 `chunk.json` 都从同一份块写出来。精读、追问、综述写作读到的仍是每段里的文字，不另存一份块文件。

一块只会是这几种：标题、段落、独立公式、夹在句子里的公式、表格、图。每块带着页码、文字，以及它在页面上的矩形。

独立公式按矩形整条收下：从最左边的公式字一直收到右边的编号 `(n)`，上下叠着的分子、分母算同一块。落在这块矩形里的字，不会再在正文里写成一对 `$`。夹在句子里的公式不另开看图：同一行里字号更小、位置更靠下的收成下标，更靠上的收成上标，所以是 `$W_{Q}$`、`$S_{m,in}$`，而不是 `$WQ$`。下一行开头就是一小截下标、又和上一行叠在一起时，接进上一截公式。

`TABLE I` 这种罗马数字表注和 `Table 2` 一样认成表。配上表注的图按整张表截图，重排成带表头的 Markdown，格子里的数字不再竖着排进正文。这种块按表装箱，不进插图清单。

独立公式、表格、图再按矩形截图，交给精读用的同一个模型看。发给 OpenAI 兼容接口时，图片用 `image_url`；Anthropic 仍用它自己的图片格式。送错格式时接口往往仍返回成功，但图根本没被看，公式就会被留成空的。认不出的公式整块退回原来的字形，包进一对 `$$`，不会把半截公式撒回段落。

`read.formula_ocr` 默认开启。关掉就跳过公式这次看图，直接走上面的退回；表格重排和插图解读没有单独开关。同一页的公式合并成一次请求。

分片顺着这些块的阅读顺序装箱，片段之间不重叠：

- 装到 1200 字就换下一段，不把一块从中间劈开。
- 新的一级标题到来、而且当前段里已经有正文时，从标题另起一段。字母小节（如 `A.`）不算一级标题。
- 单块超过 4000 字才单独成段。大表仍按行切开，每一段补上表头。

这两个数字写在 `PageChunker` 里，不通过配置文件调整。一整块不到 4000 字时可以比 1200 字长，所以不能把 1200 理解成每段的硬上限。精读读某一段时，会附带上一段末尾、下一段开头各一两句作为衔接，切片文件本身仍不重叠。

插图清单来自「图」这种块。网页全文没有这些块，才退回正文里的图片链接。

转换器和分片各自有版本号（现在是 8 和 9）。旧的 `paper.md` / `chunk.json` 对不上就重做。PyMuPDF 装不上时退回 pypdf，那条路抽不出块，分片改按空行从 Markdown 认块。
