...
> 全文分块按 PDF 页切分、片段之间不重叠：单个片段上限 1200 字符，但遇到完整表格或公式块时会整块保留（上限 4000 字符），避免把表格和公式从中间切断。这些数字写在 `PageChunker` 类里，不通过配置文件调整。精读读每一块时，会附带上一块末尾、下一块开头各一两句作为衔接，切片文件本身仍不重叠。
