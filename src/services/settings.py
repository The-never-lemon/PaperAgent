...
# 连通性测试的生成上限。
# 中文注释：思考型模型（如配置了 thinking.budget_tokens 的 DeepSeek）会把
# 思考 tokens 也计入 max_tokens——原来写死的 16 会被思考全部吃掉，正文为空，
# 明明配置可用却被判成失败。这里放大到覆盖常见思考预算（2048）再留正文余量。
CONNECTIVITY_TEST_MAX_TOKENS = 2400
...
def _agent_description(name: str, agent: AgentConfig) -> str:
    ...
    # 中文注释：系统只内置 default_agent 这一个档位，其余档位名都由用户自己写说明。
    if name.lower() == "default_agent":
        return "中等能力，适合大多数任务，默认使用，价格适中。"
...
def _provider_configured(spec: ProviderSpec, config: ProviderConfig) -> bool:
    ...
    if not (spec.default_api_base or config.api_base):
        return False
    if not _provider_requires_key(spec):
        return True
    return bool(_resolved_api_key(spec, config))


def _resolved_api_key(spec: ProviderSpec, config: ProviderConfig) -> str | None:
    """按和运行时一致的口径取密钥：先用配置里的明文，再回退到环境变量。

    中文注释：运行时（src/llm/config.py）是允许用 api_key_env 指向环境变量来提供密钥的，
    设置页的保存校验必须用同一口径，否则「只填环境变量、不填明文」的 provider
    会被误判成没配置好而拒绝保存。
    """

    return config.api_key or os.getenv(config.api_key_env or spec.env_key)
