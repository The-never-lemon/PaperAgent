...
# 中文注释：后端节点只能用 default_agent 这一个内置档位，所有子 Agent 都从它取模型。
# 主对话使用的 research_agent 不在这里，因为它由对话运行时直接指定，不经过 AgentSpec 声明。
SUPPORTED_LLM_PROFILES = frozenset({"default_agent"})
