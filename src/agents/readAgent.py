...
from src.models.read_models import ReadNote, ReadRelevance, calculate_relevance_score, normalize_match_levels
...
async def evaluate_paper_relevance(
    ...
    *,
    ...
    """对单篇论文做三维相关性评分（独立入口，供评价工具直接调用）。

    这个函数把 ReadAgent 原来"读摘要 + 算总分"的两步抽成一个工具能直接调用的入口，
    内部完全复用 build_read_agent 和 async_read_abstract，不重复实现任何阅读逻辑。
    评价工具拿到论文后直接调这里，不用自己再拼一个 Agent 上下文。

    Returns:
        {"match_levels": 三个维度的匹配档位, "score": 程序按固定表算出的总分,
         "reason": 一句话理由（取摘要笔记 short_summary）, "warning": 失败说明（成功为空串）}
    模型失败/解析失败不会往外抛异常：score 记 0，warning 说明原因。
    调用方通过 match_levels 是否为空字典区分"评分失败"和"真的全不匹配"：
    失败的论文不应该落盘成已评价状态，是否重试由调用方决定。
    """

    # 中文说明：try 里走完整的"构建阅读 Agent → 异步读摘要 → 算总分"流程。
    # 任何一步出错都折成结构化返回，不让评价工具因为单篇论文失败而中断整批。
    try:
        # 复用现成的阅读 Agent 构造方式，拿到一个绑定了传入 llm 的 ReadAgent。
        agent = build_read_agent(llm)
        # 异步读摘要：内部会自己处理空摘要（走 _metadata_only_result，三个维度全 not_match，不报错）。
        result = await agent.async_read_abstract(
            paper,
            topic=topic,
            constraints=dict(constraints or {}),
            semaphore=semaphore,
        )
        # 总分严格按固定表计算，不采用模型自报的分数，保证筛选口径一致。
        score = calculate_relevance_score(result.relevance.match_levels)
        # 一句话理由直接复用摘要笔记里的短摘要，不再额外生成。
        reason = result.note.short_summary
        # 成功时 warning 默认空串；阅读过程有 warnings 时拼进去提醒调用方。
        warning = ""
        if result.warnings:
            warning = "；".join(str(w) for w in result.warnings if str(w).strip())
        return {
        ...
        # 阅读模型不可用：最常见的失败，提醒用户去检查模型设置，分数记 0。
        return {"match_levels": {}, "score": 0, "reason": "", "warning": str(exc)}
    except Exception as exc:
        # 其他意外错误也折成结构化返回，保证评价工具的整批流程不被打断。
        # 注意：asyncio.CancelledError 继承自 BaseException，不会被这里捕获，会原样上抛。
        return {"match_levels": {}, "score": 0, "reason": "", "warning": str(exc)}
