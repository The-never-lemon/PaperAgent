...
_STAGE_DISPLAY: dict[tuple[str, str], RuntimeStageDisplay] = {
    ...
    # ------------------------------------------------------------------
    # 以下是对话式调研改造新增的映射：
    # "chat" 节点是对话主 Agent 的主卡片，每轮模型调用直接更新主卡片并携带 token 用量；
    # "tool" 节点是工具执行，每个工具名一个中文标题，
    # 同名工具多次调用靠上报时传入的 event_key 区分成不同子卡片。
    # ------------------------------------------------------------------
    ("chat", "llm_round"): RuntimeStageDisplay("chat", "对话调研", updates_parent=True),
    ("tool", "search_papers"): RuntimeStageDisplay("search_papers", "检索论文"),
    ("tool", "list_papers"): RuntimeStageDisplay("list_papers", "查询论文列表"),
    ("tool", "get_paper_details"): RuntimeStageDisplay("get_paper_details", "查看论文详情"),
