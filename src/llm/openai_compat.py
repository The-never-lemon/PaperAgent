...
class OpenAICompatProvider(LLMProvider):
    ...
    def __init__(self, *, spec: ProviderSpec, **kwargs: Any):
        ...
        if self.client is None:
            ...
            self.client = AsyncOpenAI(
                ...
                # 中文注释：重试只在项目基类（base.py 的 _run_llm_call）里做一次就够了。
                # 这里必须写成 0：如果 SDK 自己再重试，一次失败就会变成"基类重试轮数 × SDK 重试次数"，
                # 实际打出去的请求数会翻好几倍，被限流时反而更容易把上游打爆。
                max_retries=0,
            )
    ...
    async def _chat_stream_once(
        ...
        *,
        ...
        # 中文注释：流式协议里工具调用是按"分片"推送的——同一次调用先在第一个分片里
        # 给出 id 和函数名，随后用多个分片逐步拼出 arguments 字符串。这里按 index
...
def _accumulate_tool_call_delta(bucket: dict[int, JsonObject], item: JsonObject) -> None:
    """把一条流式工具调用分片按 index 聚合进 bucket。

    中文注释：OpenAI 兼容协议里，delta.tool_calls 的每个元素带 index 标记
    它属于第几个工具调用；id 和 function.name 通常只在第一个分片出现，
    function.arguments 则被拆成很多字符串分片，需要按顺序拼接。
    """

    try:
        index = int(item.get("index"))
    except (TypeError, ValueError):
        # 个别兼容网关不带 index：带 id 的分片视为一次新调用的开始，
        # 否则并入当前最后一次调用，保证分片不丢。
        index = len(bucket) if (item.get("id") or not bucket) else max(bucket)
    slot = bucket.setdefault(index, {"id": "", "name": "", "arguments_parts": []})
    if item.get("id"):
        slot["id"] = str(item["id"])
    ...
    if isinstance(arguments, str) and arguments:
        slot["arguments_parts"].append(arguments)


def _finalize_stream_tool_calls(bucket: dict[int, JsonObject]) -> list[ToolCallRequest]:
    """把聚合好的流式分片还原成完整的 ToolCallRequest 列表。

    arguments 拼接完成后尝试解析成结构化对象；非法 JSON 与非流式路径保持
    同一策略——保留原始字符串，交给上层调用方兜底，不在这一层丢弃。
    """
    ...
    for index in sorted(bucket):
        slot = bucket[index]
        if not slot["name"]:
            # 没拼出函数名的分片序列无法执行，跳过（正常协议不会出现）。
            continue
        raw_arguments = "".join(slot["arguments_parts"])
        arguments: Any
        try:
