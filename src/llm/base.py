...
# 中文注释：服务端 Retry-After 建议等待时间的上限（秒）。
# 我们原则上听服务端的（它说等多久就等多久，避免白白浪费重试），但万一某个
# 网关返回离谱的数值（比如 3600 秒），傻等一小时对用户来说就是"这次调研卡死了"。
# 所以最多按这个秒数等，到点就再试一次；还是失败就继续正常的重试节奏。
...
class LLMResponse:
    ...
    Attributes:
        ...
        reasoning_blocks: thinking 内容块的原始结构（含 signature）。推理模型要求下一轮
            请求把这些块原样回传，否则上游会以 400 拒绝，所以这里单独留着。
    ...
    reasoning_blocks: list[JsonObject] = field(default_factory=list)
...
class LLMProvider(ABC):
    def __init__(
        ...
        *,
        ...
        # 中文注释：并发上限就靠这个信号量统一控制——每个 provider 自己数着"现在有几个请求正在飞"。
        # 名额只在真正发请求那一下占用，等重试的空档会把名额还回去，让别的调用先用（见 _run_llm_call）。
    ...
    async def aclose(self) -> None:
        ...
        if self._owns_client and self.client is not None:
            # 中文注释：OpenAI 和 Anthropic 的异步客户端都只提供 close()，没有 aclose()，
            # 所以先找 aclose、找不到就用 close 兜底，避免漏关连接池。
            close = getattr(self.client, "aclose", None) or getattr(self.client, "close", None)
            if close is not None:
                result = close()
                # 中文注释：这两个 SDK 的 close() 是 async 方法，必须 await 才会真正释放连接，
                # 直接调用只会拿到一个没人执行的协程对象（并抛 RuntimeWarning）。
    ...
    async def _run_llm_call(self, operation: str, call_once: Callable[[], Awaitable[LLMResponse]]) -> LLMResponse:
        ...
        for attempt in range(self.max_retries):
            attempts = attempt + 1
            # 中文注释：并发名额（信号量）只在"真正发这一次请求"的时候占用。
            # 请求一返回（不管成功还是失败）就马上把名额还回去，
            # 后面等待重试的那段时间不占名额，别的调用可以拿这个名额去请求模型，不用陪着一起干等。
    ...
    def _retry_delay(self, attempt: int, response: LLMResponse | EmbeddingResponse | None = None) -> float:
        ...
        Args:
            attempt: 已经尝试过几次（第 1 次重试时 attempt=1）。
            response: 刚刚那次失败拿到的响应对象，可为空。里面可能带着服务端
                通过 `Retry-After` 响应头告诉我们的"请等 X 秒再来"。

        Returns:
            需要等待的秒数。

        取两者中更大的一个：我们自己算的递增等待时间，和服务端要求的等待时间。
        服务端说要等 30 秒，我们只等 1 秒就再发请求，那次请求几乎肯定还是失败，
        等于白白浪费一次调用，所以必须听服务端的。
        服务端要求的等待时间最多听到 _RETRY_AFTER_CAP_SECONDS 秒为止，
        防止离谱的 Retry-After 数值把一次调研整个卡死。
        """
        ...
        our_delay_s = (base_delay_ms + jitter_ms) / 1000
        # 服务端给的建议等待时间单位本来就是秒，直接和上面算出的秒数比大小，不用再换算。
        # 但要设个上限：最多听服务端等 _RETRY_AFTER_CAP_SECONDS 秒，防止离谱数值卡死整次调研。
        retry_after_s = response.error_retry_after_s if response is not None else None
        if retry_after_s is not None and retry_after_s > 0:
            capped_retry_after_s = min(float(retry_after_s), _RETRY_AFTER_CAP_SECONDS)
