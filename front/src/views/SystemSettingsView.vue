...
async function runConnectivityTest(targetType: "agent", name: string) {
  if (!name) {
    return;
  }
...
function connectivityLabel(targetType: "agent", name: string) {
  const key = connectivityKey(targetType, name);
...
function connectivityKey(targetType: "agent", name: string) {
  return `${targetType}:${name}`;
}

function invalidateConnectivity(targetType: "agent", name: string) {
  const key = connectivityKey(targetType, name);
